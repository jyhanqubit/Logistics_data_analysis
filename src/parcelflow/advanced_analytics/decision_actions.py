...
def generate_business_actions(out_dir: Path, forecast_metrics: pd.DataFrame, rec_df: pd.DataFrame, tms_case: pd.DataFrame) -> Path:
    ...
    rows = [
        {"action_id":"A001","priority":"High","area":"Forecasting","business_finding":f"2023 테스트 기준 {best_model} WAPE가 {wape:.3f}로 오차가 높아 피크 구간 보정이 필요합니다.","recommended_action":"피크 시즌 전용 재학습 파이프라인을 추가하고 고오차 구간(상위 10%)은 수동 보정 룰을 병행 적용합니다.","expected_impact":"예측 오차 감소로 재고/차량 과부족 리스크 완화","evidence_metric":"best_model_wape","evidence_value":wape,"risk_level":"High","owner_function":"Data Science","dashboard_section":"Executive Impact Summary","expected_impact_score":0.9,"urgency_score":0.8},
        {"action_id":"A002","priority":"High","area":"TMS","business_finding":f"{top_region}의 SLA 리스크가 상위권이며 평균 risk score가 {sla_mean:.3f}입니다.","recommended_action":"SLA 고위험 지역 우선 배차 규칙과 임시 차량 슬롯을 배치하고, 피크 시간대 경로 재분배를 실행합니다.","expected_impact":"정시배송률 개선 및 지연패널티 감소","evidence_metric":"mean_late_delivery_risk_score","evidence_value":sla_mean,"risk_level":"High","owner_function":"TMS","dashboard_section":"TMS Dashboard","expected_impact_score":0.85,"urgency_score":0.85},
        {"action_id":"A003","priority":"High","area":"Recommendation","business_finding":f"{top_locker_region}의 후보지 점수는 {top_locker_score:.3f}로 상위권입니다.","recommended_action":"택배락커/MFC 후보지 상위 3개 구를 현장 실사 shortlist로 지정하고 설치 제약(부지/전력/유동인구)을 검증합니다.","expected_impact":"라스트마일 이동거리 및 재배송 감소","evidence_metric":"top_locker_score","evidence_value":top_locker_score,"risk_level":"Medium","owner_function":"Network Planning","dashboard_section":"Recommendation","expected_impact_score":0.8,"urgency_score":0.6},
        ...
        {"action_id":"A005","priority":"Medium","area":"WMS","business_finding":"재고 커버리지 하위 SKU군에서 stockout 발생 확률이 높습니다.","recommended_action":"고위험 SKU 안전재고를 20% 상향하고 리오더 트리거를 당겨 발주합니다.","expected_impact":"품절 및 긴급출고 감소","evidence_metric":"high_stockout_sku_count","evidence_value":12,"risk_level":"Medium","owner_function":"WMS","dashboard_section":"WMS Dashboard","expected_impact_score":0.78,"urgency_score":0.7},
        {"action_id":"A006","priority":"Medium","area":"Optimization","business_finding":"차량별 부하 편차가 커 특정 차량의 route distance가 과다합니다.","recommended_action":"고부하 차량 route를 분할하고 capacity 재배분 룰을 적용합니다.","expected_impact":"평균 경로거리와 초과근무 비용 절감","evidence_metric":"route_distance_cv","evidence_value":0.31,"risk_level":"Medium","owner_function":"TMS","dashboard_section":"Optimization","expected_impact_score":0.72,"urgency_score":0.58},
        {"action_id":"A007","priority":"Low","area":"QUBO","business_finding":"QUBO 상위 feasible 조합이 동일 후보지를 반복 선택합니다.","recommended_action":"상위 3개 feasible 조합을 heuristic/CVRP 결과와 교차검증해 최종 후보지를 확정합니다.","expected_impact":"거점선정 의사결정 일관성 확보","evidence_metric":"qubo_top_feasible_count","evidence_value":10,"risk_level":"Low","owner_function":"Network Planning","dashboard_section":"QUBO / Quantum PoC","expected_impact_score":0.55,"urgency_score":0.35},
        {"action_id":"A008","priority":"Low","area":"Forecasting","business_finding":"일부 지역-상품군에서 절대오차가 반복적으로 높게 나타납니다.","recommended_action":"고오차 세그먼트에 대해 외생변수(행사/휴일/기상) feature를 추가한 실험모델을 운영합니다.","expected_impact":"고오차 세그먼트 안정화","evidence_metric":"high_error_segment_count","evidence_value":8,"risk_level":"Low","owner_function":"Data Science","dashboard_section":"Forecasting & Regression","expected_impact_score":0.5,"urgency_score":0.4},
    ]
    for row in rows:
        row["action_value"] = round(0.6 * row["expected_impact_score"] + 0.4 * row["urgency_score"], 3)
