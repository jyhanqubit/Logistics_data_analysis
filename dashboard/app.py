...
with tabs[0]:
    ...
    c1.metric("Forecasting Best WAPE", f"{best_wape:.2%}" if best_wape is not None else "N/A")
    c2.metric("Peak Risk Regions", str(peak_regions))
    c3.metric("High Stockout Risk Categories", str(high_stock))
    c4.metric("High SLA Risk Regions", str(high_sla))
    c5.metric("Recommended Locker Sites", str(reco_sites))
    c6.metric("Route Distance Saving", f"{save:.2%}" if save is not None else "N/A")

    st.markdown("### Top 5 Recommended Actions")
    st.dataframe(insights_actions.head(5) if not insights_actions.empty else pd.DataFrame({"message": ["actions 없음"]}), use_container_width=True)

    st.markdown("### Action Priority Matrix")
    st.dataframe(priority if not priority.empty else pd.DataFrame({"message": ["priority matrix 없음"]}), use_container_width=True)
    ...
    st.markdown("### Action Value 계산 의미")
...
with tabs[1]:
    st.markdown(read_md(DOCS / "dashboard_column_dictionary.md", "column dictionary 문서 없음"))
    st.markdown("### 실제 분석 사용 컬럼 명세")
...
with tabs[3]:
    ...
    if not inv.empty and "stockout_risk_score" in inv.columns:
        st.bar_chart(inv.sort_values("stockout_risk_score", ascending=False).head(10).set_index("product_category")["stockout_risk_score"])
    if not pick.empty and "warehouse_utilization" in pick.columns:
        st.bar_chart(pick.groupby("warehouse_name")["warehouse_utilization"].mean())
...
with tabs[5]:
    ...
    if not reg_metrics.empty and {"model", "wape"}.issubset(reg_metrics.columns):
        st.bar_chart(reg_metrics.set_index("model")[["wape", "rmse"]])
...
with tabs[6]:
    ...
    if not cls_metrics.empty and {"task", "f1"}.issubset(cls_metrics.columns):
        st.bar_chart(cls_metrics.groupby("task")[["precision", "recall", "f1"]].mean())
...
with tabs[7]:
    ...
    if not stat_group.empty and {"group", "mean"}.issubset(stat_group.columns):
        st.bar_chart(stat_group.set_index("group")["mean"])
...
with tabs[8]:
    ...
    if not acf.empty and {"lag", "correlation", "function_type"}.issubset(acf.columns):
        st.bar_chart(acf[acf["function_type"] == "ACF"].set_index("lag")["correlation"].head(28))
        st.bar_chart(acf[acf["function_type"] == "PACF"].set_index("lag")["correlation"].head(28))
